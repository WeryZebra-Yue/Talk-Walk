  ...
  return (
    <div className="flex flex-col w-full unselectable ">
      <Head>
        ...
        <meta
          property="og:url"
          content={`https://is-clone3.vercel.app/post/${id}`}
        />
      ...
      <div className="flex flex-col pb-2 justify-between items-center rounded-b-2xl bg-white shadow-md text-gray-400 border-t">
        ...
        <div className="flex w-full">
          {" "}
          {session && <Like id={id} />}
          {!session && <PageLike id={id} />}
          {/* ,,,,,,,,,,,,,,,,,,,,,,,,,,,, */}
          <a
            className="inputIcon rounded-none unselectable"
            href={`whatsapp://send?text=https://is-clone3.vercel.app/post/${id}`}
            data-action="share/whatsapp/share"
          >
            <ShareIcon className="h-4" />
            <p className="text-xs sm:text-base" id={`share${id}`}>
              Share
            </p>
          </a>
        </div>
        {!session && (
          <p>
            <Link href="/">Sign in</Link> to Like and See More Post!
          </p>
        )}
      </div>
    </div>
